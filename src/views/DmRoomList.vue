      ...
      <div v-else class="flex flex-col gap-1">
        ...
        >
          ...
          <div class="flex-1 min-w-0 ml-4 flex flex-col">
            <span class="font-semibold text-white text-[17px] truncate">{{ dm.targetUserNickname }}</span>
            <div class="flex items-center mt-1 gap-2">
              ...
              <span class="text-xs text-gray-500 ml-2 whitespace-nowrap">{{ relativeTime(dm.lastMessageTimestamp) }}</span>
            </div>
          </div>
          <i class="fas fa-chevron-right text-gray-500 group-hover:text-orange-400 ml-3 text-base"></i>
        </button>
      </div>
    </div>
  </div>
