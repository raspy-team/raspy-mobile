...
function goToProfile() { router.push(`/profile/${targetUserId}`) }
    ...
    <div class="fixed left-0 w-full h-[110px] z-20" :style="{ top: `${headerTop}px` }">
      <div class="h-full flex items-end p-4 bg-white shadow gap-2 z-20">
        ...
        <div class="flex items-center gap-2" @click="goToProfile">
          <img :src="targetUserProfileUrl || defaultProfileUrl" alt="상대방 프로필"
      ...
      <div ref="chatBox" class="h-full overflow-auto space-y-4 pb-2">
        ...
        <template v-for="(msg, idx) in visibleMessages" :key="msg._key">
          <div v-if="shouldShowDate(idx, visibleMessages)" class="flex justify-center mb-1">
            <div class="bg-white border px-4 py-1 rounded-full text-xs text-gray-500 shadow-sm font-medium">
              {{ formatDateOnly(msg.timestamp) }}
            </div>
          </div>
          <div class="flex" :class="msg.senderId === currentUserId ? 'justify-end' : 'justify-start'">
            <template v-if="msg.messageType === 'TALK'">
              <div v-if="msg.senderId === currentUserId" class="flex items-end gap-2">
                <div class="max-w-xs p-3 rounded-xl break-words flex-row-reverse bg-orange-500 text-white rounded-br-none flex flex-col">
                  <p class="whitespace-pre-line leading-relaxed flex-1">{{ msg.content }}</p>
                  <p class="text-[10px] ml-2 mt-1 text-right text-gray-100">{{ formatTime(msg.timestamp) }}</p>
                </div>

              </div>
              <div v-else class="max-w-xs p-3 rounded-xl break-words bg-white text-gray-900 rounded-bl-none shadow flex flex-col items-start">
                <p class="whitespace-pre-line leading-relaxed flex-1">{{ msg.content }}</p>
                <p class="text-[10px] mt-1 text-right text-gray-400">{{ formatTime(msg.timestamp) }}</p>
              </div>
            </template>
            <template v-else>
              <div class="mx-auto bg-gray-100 px-3 py-1 rounded text-gray-600 text-xs shadow">{{ msg.content }}</div>
            </template>
          </div>
        </template>
      </div>
